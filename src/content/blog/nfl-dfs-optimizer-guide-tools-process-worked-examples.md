...
### How does an NFL DFS optimizer actually work?

An optimizer solves a linear programming problem: given a set of players with projected points and salaries, plus constraints (roster size, position requirements, salary cap), it returns the combination that maximizes total projected points. That's the entire mechanical process — most tools use the same class of solver algorithm, so the math itself isn't a differentiator. The optimizer has no opinion on whether your projections are actually right, and it ignores variance, ownership, and correlation on its own. It's a constraint solver, not a strategy engine — if your projections are off or your rules are lazy, it will faithfully return bad lineups anyway.

### How do I configure exposure caps and stacking rules in an NFL DFS optimizer?

Start exposure caps from the size of a player's projection edge, not a flat default: if he projects 3+ points ahead of the next option at his position, exposure can approach or exceed his projected ownership. If the edge is under 1.5 points, keep exposure well below ownership — you're not compensated enough for the duplication. For stacking, a QB + primary pass-catcher is the baseline; only add a bring-back from the opposing team when both offenses are actually live (implied totals both meaningfully above the field). Concentrate correlation rules in the highest-total, closest-spread games on the slate — a lopsided blowout script doesn't produce the same multi-player ceiling outcomes a genuine shootout does.

### Should I use my optimizer's built-in projections, or build my own?

Built-in projections are a reasonable starting point, but they're the same numbers every other user of that tool sees — in a large-field tournament, that overlap works against you. You don't need to build projections from scratch to get an edge. Even a small, deliberate adjustment — discounting a Sunday-morning injury downgrade, bumping a backup's projected volume after a role change, factoring in a real wind forecast at an outdoor stadium — is usually enough to move your build off the most common lineup combinations other users are generating from the same default numbers.
